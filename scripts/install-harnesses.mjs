...
import { getLocalLanIp, getTailscaleIp } from "../src/shared/local-addresses.mjs";
import { mcpServersRecipeJson } from "../src/shared/mcp-recipe.mjs";
...
const SKIP_RUNNING_STATUS = process.argv.includes("--skip-running-status");
  ...
  if (LIST_HARNESSES_MODE) {
    console.log(JSON.stringify(ALL_HARNESSES.filter((harness) => harness.id !== "manual").map((harness) => {
      ...
      const manualRestartRequired =
        !SKIP_RUNNING_STATUS && harness.id === "codex" && interactiveCodexCliIsRunning();
      const restartTarget =
        !SKIP_RUNNING_STATUS && restartSpec && !manualRestartRequired ? detectRestartTarget(restartSpec) : null;
      return {
        id: harness.id,
        name: harness.name,
        group: harness.group,
...
function windowsProcessIsRunning(name) {
  const processName = windowsExeName(name).toLowerCase();
  if (!windowsProcessNames) {
    const result = spawnSync("tasklist.exe", ["/FO", "CSV", "/NH"], {
      encoding: "utf8",
      stdio: "pipe",
      shell: false,
    });
    windowsProcessNames = new Set(
      result.status === 0
        ? result.stdout.split(/\r?\n/).map((line) => line.match(/^"([^"]+)"/)?.[1]?.toLowerCase()).filter(Boolean)
        : []
    );
  }
  return windowsProcessNames.has(processName);
}
...
function printManualMcpRecipe(harness, serverEntry) {
  ...
  console.log(mcpServersRecipeJson(serverEntry, SERVER_NAME));
}
...
async function run(command, args, options = {}) {
  ...
  const streamOutput = process.env.ROBLOX_MCP_INSTALLER_STREAM_OUTPUT === "1";
  if (streamOutput) console.log(`$ ${[command, ...args.map(quote)].join(" ")}`);
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        spinner.stop(false);
        const details = !streamOutput && output.trim() ? `\n\n${output.trim()}` : "";
