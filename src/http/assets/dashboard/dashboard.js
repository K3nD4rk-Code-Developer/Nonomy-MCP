...
let decompilerAdvancedOpen = false;
...
async function saveDecompilerProviderModal(options = {}) {
    ...
    if (id === 'oracle') {
        ...
        try {
            ...
            const transport = ['http', 'websocket'].includes($('decompilerModalLuacidTransport')?.value)
                ? $('decompilerModalLuacidTransport').value
                : 'auto';
            provider.options = {
                ...parsed,
                transport,
            ...
            setDecompilerProviderSaveStatus('Not saved: options must be a JSON object.', true);
            if (!options.silent) showToast('Luacid options must be a JSON object', 'error');
            return false;
        }
...
$('settingsAddDecompilerMenu').addEventListener('click', (e) => {
    ...
    $('settingsAddDecompilerMenu').classList.remove('open');
    activateDecompilerProvider(id);
    if (id === 'oracle' || id === 'luacid' || id === 'fission' || id === 'shiny' || isCustomDecompilerProviderId(id)) {
        openDecompilerProviderModal(id);
    } else {
        scheduleDecompilerAutoSave();
    }
