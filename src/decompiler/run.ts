...
function luacidOptions(
  ...
  return Object.fromEntries(Object.entries(provider.options).filter(
    ([key, value]) => !["transport", "transportExplicit"].includes(key) && ["string", "number", "boolean"].includes(typeof value)
...
async function runLuacid(
  ...
  const explicitHttp = provider.options.transport === "http" && provider.options.transportExplicit === true;
  const useWebSocket = provider.options.transport === "websocket" ||
    (!explicitHttp && Boolean(provider.apiKey));
  if (!useWebSocket) {
  ...
  return new Promise((resolve) => {
    ...
    const url = withQuery(websocketEndpoint, options);
    const socket = new WebSocket(url, { headers: { Authorization: `Bearer ${provider.apiKey}` } });
    let settled = false;
    ...
    socket.once("message", (data) => {
      try {
        ...
        if (typeof response.decompilation === "string") {
          ...
          finish({
            ok: false,
            error: typeof response.error?.message === "string" ? response.error.message : "Invalid Luacid WebSocket response.",
            latencyMs: Date.now() - startedAt,
          });
        }
      } catch {
        finish({ ok: false, error: "Invalid Luacid WebSocket response.", latencyMs: Date.now() - startedAt });
      }
    });
    socket.once("error", (error) => finish({ ok: false, error: error.message, latencyMs: Date.now() - startedAt }));
  });
}
